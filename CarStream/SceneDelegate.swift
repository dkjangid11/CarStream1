...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func sceneDidEnterBackground(_ scene: UIScene) {
       ...
       func sceneDidBecomeActive(_ scene: UIScene) {
           ...
           let userDefaults = UserDefaults(suiteName: "group.net.thomasdye.CarStream-docs")
...
//               userDefaults?.addObserver(self, forKeyPath: "TDSSharedURL", options: .new, context: nil)
    ...
    func scene(_ scene: UIScene, openURLContexts URLContexts: Set<UIOpenURLContext>) {
        ...
        if url.scheme?.lowercased() == "tdsvideopayment" {
            CarStreamVideoAPI.shared.HidePaymentScreen()
    ...
    private func handleIncomingSharedURL(_ url: URL) {
        ...
        CarStreamVideoShared.shared.CarPlayComp?(.init(type: .web, URL: url))
    }
