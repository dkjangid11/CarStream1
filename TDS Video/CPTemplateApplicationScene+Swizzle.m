...
+ (void)load {
    ...
    dispatch_once(&onceToken, ^{
        ...
        TDSSwizzleInstanceMethod(cls,
                                 ...
                                 @selector(carstream_trace_shouldCreateCarWindow));

        TDSSwizzleInstanceMethod(cls,
                                 NSSelectorFromString(@"_attachWindow:"),
                                 @selector(carstream_trace_attachWindow:));

        TDSSwizzleInstanceMethod(cls,
                                 NSSelectorFromString(@"setCarWindow:"),
                                 @selector(carstream_trace_setCarWindow:));

        TDSSwizzleInstanceMethod(cls,
                                 NSSelectorFromString(@"_deliverInterfaceControllerToDelegate"),
                                 @selector(carstream_trace_deliverInterfaceControllerToDelegate));
    });
}

- (BOOL)carstream_trace_shouldCreateCarWindow {
    NSLog(@"[CarPlayTrace] ENTER _shouldCreateCarWindow scene=%@", self);

    BOOL result = [self carstream_trace_shouldCreateCarWindow];
...
+ (void)load {
    ...
    dispatch_once(&onceToken, ^{
        TDSSwizzleInstanceMethod(self,
                                 ...
                                 @selector(carstream_trace_initWithFrame:templateScene:));
    });
}

- (instancetype)carstream_trace_initWithFrame:(CGRect)frame templateScene:(id)scene {
    NSLog(@"[CarPlayTrace] ENTER CPWindow initWithFrame:%@ templateScene=%@",
          NSStringFromCGRect(frame),
          scene);

    id window = [self carstream_trace_initWithFrame:frame templateScene:scene];
