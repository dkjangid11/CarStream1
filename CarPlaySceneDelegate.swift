...
class CarPlaySceneDelegate: UIResponder, CPTemplateApplicationSceneDelegate, CPMapTemplateDelegate, CPSearchTemplateDelegate {
    ...
    private var youtubeSearchResults: [CarStreamYouTubeSearchResult] = []
...
//        interfaceController.setRootTemplate(CPGridTemplate(title: "TEST", gridButtons: []), animated: true)
        ...
        if CarStreamAccess.shared.ShowCarStreamSettings == false  {
...
//        self.interfaceController._carwin
        ...
        CarStreamVideoShared.shared.window = window
        print("keys")
        ...
        if let rootView = self.window?.rootViewController?.view {
            CarStreamAirPlayReceiverManager.shared.registerCarPlayContainer(rootView)
        }
        //        print(self.window?.screen)
//        DispatchQueue.main.asyncAfter(deadline: .now() + 5, execute: {
//            //            self.loadWebPage(url: URL(string: "https://thomasdye.net")!)
//        })
        CarStreamVideoShared.shared.CarPlayComp = { url in
        ...
        CarStreamVideoURlFromOutSideOFAppListener.shared.onUpdate = {url in
            print(url)
    ...
    func templateApplicationScene(_ templateApplicationScene: CPTemplateApplicationScene,
        ...
        CarStreamAirPlayReceiverManager.shared.unregisterCarPlayContainer()
...
//        })
    ...
    private func makeYouTubeHomeMapButton() -> CPMapButton {
        let button = CPMapButton { [weak self] _ in
            ...
            if self.youtubeNativeRecommendationsEnabled {
                ...
                CarStreamControlCommandCenter.shared.run(.openYouTube)
            }
        }
    ...
    private func makeYouTubeSearchItem(
        for result: CarStreamYouTubeSearchResult,
        includeHandler: Bool
    ) -> CPListItem {
    ...
    private func loadRecommendationImages(
        _ recommendations: [CarStreamYouTubeSearchResult],
        into template: CPListTemplate,
        generation: Int
    ) {
    ...
    private func makeRecommendationImageRow(
        videos: [CarStreamYouTubeSearchResult],
        images: [UIImage]
    ) -> CPListImageRowItem {
    ...
    private func makeMapButton(systemImage: String, command: CarStreamControlCommand) -> CPMapButton {
        let button = CPMapButton { _ in
            print("CarPlay map button tapped command=\(command.rawValue)")
            CarStreamControlCommandCenter.shared.run(command)
        }
        button.image = UIImage(systemName: systemImage)
        return button
    }

    private func makeBarButton(title: String, command: CarStreamControlCommand) -> CPBarButton {
    ...
    func loadIOS() {
        ...
        for subview in window!.subviews {
         ...
         if let rootViewController = self.window?.rootViewController {
             ...
             CarStreamAirPlayReceiverManager.shared.registerCarPlayContainer(rootViewController.view)
         }
