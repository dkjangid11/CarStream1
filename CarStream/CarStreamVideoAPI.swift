...
class CarStreamVideoAPI:NSObject,ObservableObject {
    static let shared = CarStreamVideoAPI()
    ...
    private let callCountKey = "CarStreamVideoAPICallCount"
...
//        request.setValue("Bearer \(auth.GetToken())", forHTTPHeaderField: "Authorization")
        ...
        let latitude = CarStreamLocationAPI.shared.latitude
        let longitude = CarStreamLocationAPI.shared.longitude
...
//
    ...
    func sendEmail() {
        let subject = "CarStream Support Request device UUID: \(UIDevice.current.identifierForVendor?.uuidString ?? "")"
          let body = ""
...
extension CarStreamVideoAPI: URLSessionDelegate {
