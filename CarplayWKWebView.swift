...
struct CarStreamYouTubeSearchResult {
    let id: String
    let title: String
    let channel: String
    let details: String
...
class CustomWebViewController: CarPlayViewControllerProtocol, WKNavigationDelegate, WKUIDelegate, UIGestureRecognizerDelegate, WKScriptMessageHandler {
    ...
    private weak var browserStandbyView: CarStreamBrowserStandbyView?
                    ...
                    (function() {
                        ...
                        window.tdsPostFullscreenRequest = function(element, target) {
                            try {
                                ...
                                console.log('CarStream fullscreen request post failed', error);
                            }
                        };
                        ...
                        window.tdsRequestVideoFullscreen = function(video) {
                            ...
                            try {
                                ...
                                console.log("CarStream fullscreen request failed", error);
                                window.tdsEnterCssFullscreen(video);
                            }
                        };
...
//             webViewController.view.isUserInteractionEnabled = true
    ...
    private func installCarPlayGestureLogging(rootView: UIView) {
        ...
        print("CarStream CarPlay gesture logging installed")
    }
    ...
    @objc private func logCarPlayTap(_ gesture: UITapGestureRecognizer) {
        ...
        print("CarStream CarPlay gesture tap name=\(gesture.name ?? "unknown") taps=\(gesture.numberOfTapsRequired) point=\(point) windowPoint=\(windowPoint)")
    }
    ...
    @objc private func logCarPlayPan(_ gesture: UIPanGestureRecognizer) {
        ...
        print("CarStream CarPlay gesture pan name=\(gesture.name ?? "unknown") state=\(state) point=\(point) translation=\(translation) velocity=\(velocity)")
    }
    ...
    func searchYouTube(
        for query: String,
        completion: @escaping (Result<[CarStreamYouTubeSearchResult], Error>) -> Void
    ) {
        loadViewIfNeeded()
        ...
        Task { @MainActor in
            do {
                ...
                let items = rows.compactMap { row -> CarStreamYouTubeSearchResult? in
                    ...
                    return CarStreamYouTubeSearchResult(
                        id: id,
                        title: title,
    ...
    private static func youtubeVideoItems(from value: Any?) -> [CarStreamYouTubeSearchResult] {
        let rows = value as? [[String: Any]] ?? []
    ...
    private func updateCarPlayBrowserStandby(in rootView: UIView) {
        ...
        let standby = CarStreamBrowserStandbyView()
    ...
    func select() {
        ...
        webView.evaluateJavaScript(js) { result, error in
            if let error {
                print("CarStream CarPlay cursor click failed: \(error.localizedDescription)")
            } else {
                print("CarStream CarPlay cursor click result: \(String(describing: result))")
            }
        }
    }
    ...
    func scrollBy(x: CGFloat, y: CGFloat) {
        ...
        webView?.evaluateJavaScript(js) { result, error in
            if let error {
                print("CarStream CarPlay web scroll failed: \(error.localizedDescription)")
            } else {
                print("CarStream CarPlay web scroll dx=\(x) dy=\(y) result=\(String(describing: result))")
            }
        }
    }
    ...
    func webView(_ webView: WKWebView, decidePolicyFor navigationAction: WKNavigationAction, decisionHandler: @escaping (WKNavigationActionPolicy) -> Void) {
        if let url = navigationAction.request.url {
            ...
            if IsIncar, isDirectPlayableMediaURL(url) {
                CarStreamVideoShared.shared.CarPlayComp?(.init(type: .video, URL: url))
                ...
                if let currentURL = webView.url?.absoluteString {
                    ...
                    if isYouTubeURL(webView.url) {
                        webView.evaluateJavaScript("""
                            ...
                            """) { result, error in
                                if let error {
                                    print("CarStream YouTube picker bootstrap failed: \(error.localizedDescription)")
                                } else {
                                    print("CarStream YouTube picker bootstrap: \(String(describing: result))")
                                }
                            }
        ...
        for delay in delays {
            DispatchQueue.main.asyncAfter(deadline: .now() + delay) { [weak self] in
                ...
                (function() {
                    ...
                    if (moviePlayer && typeof moviePlayer.playVideo === 'function') {
                        ...
                        if (result && typeof result.catch === 'function') {
                            result.catch(error => console.log('CarStream native selection play failed', error));
                        }
                    }
                ...
                self.webView?.evaluateJavaScript(js) { result, error in
                    if let error {
                        print("CarStream selected YouTube video play failed: \(error.localizedDescription)")
                    } else {
                        print("CarStream selected YouTube video play result: \(String(describing: result))")
                    }
                }
            }
        }
    ...
    func toggleCurrentPageVideoPlayback() {
        ...
        (function() {
                ...
                function enforceYouTubeState() {
                    ...
                    if (request.desiredPaused) {
                        ...
                        if (playResult && typeof playResult.catch === 'function') {
                            playResult.catch(error => console.log('CarStream YouTube play failed', error));
                        }
                    }
                }
            ...
            if (wasPaused) {
                ...
                const playResult = video.play();
                if (playResult && typeof playResult.catch === 'function') {
                    playResult.catch(error => console.log('CarStream play failed', error));
                }
                refreshVideoLayout();
                [100, 300, 700, 1200].forEach(delay => setTimeout(refreshVideoLayout, delay));
                return 'Play';
        ...
        webView?.evaluateJavaScript(js) { result, error in
            if let error {
                print("CarStream CarPlay play/pause failed: \(error.localizedDescription)")
            } else {
                print("CarStream CarPlay play/pause result: \(String(describing: result))")
            }
        }
    }
    ...
    private func runYouTubeItemSelection(action: String) {
        ...
        webView?.evaluateJavaScript(js) { result, error in
            if let error {
                print("CarStream CarPlay YouTube \(action) failed: \(error.localizedDescription)")
            } else {
                print("CarStream CarPlay YouTube \(action) result: \(String(describing: result))")
            }
        }
    }
    ...
    func setYouTubeCustomPickerEnabled(_ enabled: Bool) {
        ...
        webView?.evaluateJavaScript(js) { result, error in
            if let error {
                print("CarStream YouTube picker preference failed: \(error.localizedDescription)")
            } else {
                print("CarStream YouTube picker preference: \(String(describing: result))")
            }
        }
        applyYouTubePageZoom(for: webView?.url)
    }
    ...
    func setYouTubePickerSearchEnabled(_ enabled: Bool) {
        ...
        webView?.evaluateJavaScript(js) { result, error in
            if let error {
                print("CarStream YouTube picker search preference failed: \(error.localizedDescription)")
            } else {
                print("CarStream YouTube picker search preference: \(String(describing: result))")
            }
        }
    }
    ...
    func inspectCurrentVideoOverlays(completion: @escaping (String) -> Void) {
        ...
        (function() {
            ...
            return report.length ? 'CarStream overlay inspector v' + inspectorVersion + '\\n' + report.join('\\n') :
                'CarStream overlay inspector v' + inspectorVersion + ': No overlay candidates found above the video';
            } catch (error) {
        ...
        webView?.evaluateJavaScript(js) { result, error in
            ...
            print("CarStream video overlay inspection:\n\(report)")
    ...
    private func applyYouTubePageZoom(for url: URL?) {
        ...
        if !isPlaybackPage {
            webView.evaluateJavaScript(
                ...
                if let error {
                    print("CarStream YouTube fullscreen cleanup failed: \(error.localizedDescription)")
                }
            }
        }
    ...
    func userContentController(_ userContentController: WKUserContentController, didReceive message: WKScriptMessage) {
        if message.name == "tdsYouTubePickerStatus" {
            if let status = message.body as? [String: Any] {
                ...
                print("CarStream YouTube picker status state=\(state) cards=\(count) path=\(path) detail=\(detail)")
            } else {
                print("CarStream YouTube picker status: \(message.body)")
            }
            return
        }
        ...
        print("CarStream fullscreen requested: \(message.body)")

        if let currentURL {
            CarStreamVideoShared.shared.CarPlayComp?(.init(type: .web, URL: currentURL, reloadWeb: false))
        } else {
            CarStreamVideoShared.shared.CarPlayComp?(.init(type: .web, URL: nil, reloadWeb: false))
        }
